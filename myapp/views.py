...
def webcam_qr_code_scanner(request):
    if request.method == 'POST':
        ...
        if decoded_objects:
            ...
            #FOR TIMEOUT
            if "15:30" <= prac_time <= "23:59" and temporay.objects.filter(EmpCode_id=EmpCode, timein_names__isnull=False, breakout_names__isnull=False,breakin_names__isnull=False, timeout_names__isnull=True,date=current_time.date()).exists():
...
def deleteTable():
        temporay.objects.exclude(date=date.today()).delete()
